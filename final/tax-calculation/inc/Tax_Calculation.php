...
class Tax_Calculation   {
	...
	private $elect_auth_search = array('eng'=>'no','sl'=>'nie');
	...
	private function set_price_abroad_1($auth,$send_doc){
		...
		elseif(preg_match("/{$this->elect_auth_search['sl']}/i", strtolower($auth))):
	...
	private function set_price_local($field,$type)	{
		...
		elseif(preg_match("/{$this->elect_auth_search['sl']}/i", strtolower($field['elect_auth']))):
	...
	function price_table_row($fields,$type,$lang){
		...
		elseif(preg_match("/{$this->elect_auth_search['sl']}/i", strtolower($fields['elect_auth']))):
